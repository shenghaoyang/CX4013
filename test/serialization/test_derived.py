...
import pytest
from serialization.derived import create_struct_type, create_array_type, create_union_type, create_enum_type, String
from serialization.numeric import i64, u64


Point = create_struct_type('Point', (
    ('x', i64), ('y', i64)
))
PointArray = create_array_type('Point', Point)
PointOrPointArray = create_union_type('PointOrPointArray', (
    ('point', Point), ('point_array', PointArray)
))
...
def point_equals(p1: Point, p2: Point) -> bool:
    return ((p1['x'].value == p2['x'].value) and
            (p1['y'].value == p2['y'].value))


def point_array_equals(a1: PointArray, a2: PointArray) -> bool:
    return ((len(a1) == len(a2)) and
            all(point_equals(v1, v2) for v1, v2 in zip(a1, a2)))
...
class TestString:
    ...
    def test_size_property(self, simple_string: String):
        assert simple_string.size == (u64().size + len(simple_string.value.encode('UTF-8')))
