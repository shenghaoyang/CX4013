...
def create_struct_type(name: str, elements: Sequence[Tuple[str, Type[Serializable]], ...] = tuple()) -> Type[Struct]:
    """
...
class Array(Serializable, MutableSequence[Serializable]):
    ...
    def __getitem__(self, i: Union[int, slice]) -> Union[MutableSequence[Serializable], Serializable]:
        return self._values[i]

    def __setitem__(self, i: Union[int, slice], o: Union[Serializable, Iterable[Serializable]]):
        sto = (self._type_check(o) if isinstance(i, int) else
               (self._type_check(v).copy() for v in o))
...
def create_array_type(elem_type_name: str, elem_type: Type[Serializable]) -> Type[Array]:
    """
...
def create_union_type(name: str, elements: Sequence[Tuple[str, Type[Serializable]], ...] = tuple()) \
        -> Type[Union_]:
    """
    ...
    return cast(Type[Union_], type(name, (Union_, ),
...
class Enum_(Serializable):
    ...
    def __init__(self, value: Optional[VALUES] = None):
        ...
        self.value = next(iter(cast(Iterable[self.VALUES], self.VALUES))) if value is None else value
...
def create_enum_type(name: str, values: Sequence[str]) -> Type[Enum_]:
    ...
    return cast(Type[Enum_], type(name, (Enum_, ), {'VALUES': enum, 'INDEX_TO_VALUES': index_to_values}))
